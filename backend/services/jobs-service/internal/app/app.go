...
func NewApp(cfg *config.Config) (*App, error) {
	...
	if cfg.LDFlag_UsingIsolatedSchema {
		...
		utils.Logger.Infof("Using isolated schema for jobs-service; role=%s", strings.ToLower(cfg.UniqueRunnerID+"-"+cfg.UniqueRunNumber))
	} else {
		utils.Logger.Info("Isolated schema disabled; using public schema for jobs-service.")
	}
	...
	for i := 1; i <= maxRetries; i++ {
		...
		if err == nil {
			utils.Logger.Infof("jobs-service connected to DB on attempt %d", i)
			break
		}
...
func (a *App) Close() {
	if a.DB != nil {
		a.DB.Close()
		utils.Logger.Info("jobs-service DB connection closed.")
	}
}

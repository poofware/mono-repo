...
func LoadConfig() *Config {
    ...
    if !ok || dbURL == "" {
        utils.Logger.Fatalf("DB_URL not found in HCP (%s)", appSecretsName)
    }
    ldSDKKey, ok := appSecrets["LD_SDK_KEY"]
    if !ok || ldSDKKey == "" {
        utils.Logger.Fatalf("LD_SDK_KEY not found in HCP (%s)", appSecretsName)
    }

    // Twilio
    twilioSID, ok := appSecrets["TWILIO_ACCOUNT_SID"]
    if !ok || twilioSID == "" {
        utils.Logger.Fatal("TWILIO_ACCOUNT_SID missing in job-service secrets")
    }
    twilioToken, ok := appSecrets["TWILIO_AUTH_TOKEN"]
    if !ok || twilioToken == "" {
        utils.Logger.Fatal("TWILIO_AUTH_TOKEN missing in job-service secrets")
    }

    // SendGrid
    sgAPIKey, ok := appSecrets["SENDGRID_API_KEY"]
    if !ok || sgAPIKey == "" {

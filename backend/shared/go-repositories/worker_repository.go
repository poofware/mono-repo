...
func (r *workerRepo) GetActiveWorkerCount(ctx context.Context) (int, error) {
	var count int
	err := r.db.QueryRow(ctx, "SELECT COUNT(*) FROM workers WHERE on_waitlist = FALSE").Scan(&count)
	return count, err
}

...
#  • Sets DEVOPS_TOOLKIT_PATH – directory of this file (always succeeds)
...
DEVOPS_TOOLKIT_PATH := $(abspath $(dir $(lastword $(MAKEFILE_LIST))))
export DEVOPS_TOOLKIT_PATH

# ── 2. establish REPO_ROOT if caller hasn’t ──────────────────────────────────
ifeq ($(origin REPO_ROOT), undefined)
  # first try Git
  REPO_ROOT := $(shell git -C $(DEVOPS_TOOLKIT_PATH) rev-parse --show-toplevel 2>/dev/null)

  ifeq ($(REPO_ROOT),)                     # git failed → fallback + warning
    REPO_ROOT := $(abspath $(DEVOPS_TOOLKIT_PATH)/..)
    yellow := \033[33m
    normal := \033[0m
...
MAKEFLAGS += -I$(DEVOPS_TOOLKIT_PATH)
